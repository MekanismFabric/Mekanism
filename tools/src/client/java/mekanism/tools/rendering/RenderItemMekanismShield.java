...
public class RenderItemMekanismShield {

    private static final MinecraftClient client = MinecraftClient.getInstance();
    private static final Map<ItemMekanismShield, ShieldTextures> shieldTexturesMap = new HashMap<>();

    public static Set<ItemMekanismShield> initShieldTextureMap() {
        if (shieldTexturesMap.isEmpty()) {
            shieldTexturesMap.put(ToolsItems.BRONZE_SHIELD, ShieldTextures.BRONZE);
            shieldTexturesMap.put(ToolsItems.LAPIS_LAZULI_SHIELD, ShieldTextures.LAPIS_LAZULI);
            shieldTexturesMap.put(ToolsItems.OSMIUM_SHIELD, ShieldTextures.OSMIUM);
            shieldTexturesMap.put(ToolsItems.REFINED_GLOWSTONE_SHIELD, ShieldTextures.REFINED_GLOWSTONE);
            shieldTexturesMap.put(ToolsItems.REFINED_OBSIDIAN_SHIELD, ShieldTextures.REFINED_OBSIDIAN);
            shieldTexturesMap.put(ToolsItems.STEEL_SHIELD, ShieldTextures.STEEL);
        }

        return shieldTexturesMap.keySet();
    }

    private static Optional<ShieldTextures> getShieldTexture(ItemMekanismShield shieldItem) {
    ...
    public static void render(ItemStack stack, ModelTransformationMode mode, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int lightLevel, int overlay) {
        Item item = stack.getItem();
        if (!(item instanceof ItemMekanismShield)) return;

        ItemMekanismShield shieldItem = (ItemMekanismShield) item;
